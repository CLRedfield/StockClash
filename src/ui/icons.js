// ====================== 图标系统（Lucide 内联 SVG） ======================
// 设计系统指定 Lucide（2px stroke, 24px grid, 圆角端点）。这里内联所需图标，
// 免依赖、免 CDN；与品牌「terminal-clean、几何 sans」一致。currentColor 继承。
// 生产可改为自托管 lucide npm 包。
...
// 返回一个 <span class="sc-ico"> 包裹真实 SVG 的 DOM 节点。
export function icon(name, { size = 18, stroke = 2, cls = '' } = {}) {
  const span = document.createElement('span');
  span.className = 'sc-ico' + (cls ? ' ' + cls : '');
  span.style.width = size + 'px';
  span.style.height = size + 'px';
  span.innerHTML = svgMarkup(name, stroke);
  return span;
}

// 直接返回 SVG 字符串（用于 innerHTML 拼装场景）。
export function svgMarkup(name, stroke = 2) {
